...
function TrackOrder() {
  ...
  useEffect(() => {
    const getOrder = async () => {
      try {
        ...
        const coords = orderData.assignedDeliveryBoy?.location?.coordinates
        if (coords?.length === 2) {
          setDeliveryBoyLocation({
            latitude: coords[1],
            longitude: coords[0]
  ...
  useEffect(() => {
    if (!order?._id || !order?.assignedDeliveryBoy?._id) return
  ...
  }, [order?._id, order?.assignedDeliveryBoy?._id])
          ...
          <div className='bg-white rounded-3xl shadow-lg border p-4 h-[430px] flex flex-col'>
            ...
            <div className='flex-1 overflow-y-auto p-2 space-y-3' ref={chatBoxRef}>
              <AnimatePresence>
                {messages?.map((msg) => (
                  <motion.div
                    key={msg._id?.toString() || `${msg.senderId}-${msg.time}`}
                    initial={{ opacity: 0, y: 15 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0 }}
                    transition={{ duration: 0.2 }}
